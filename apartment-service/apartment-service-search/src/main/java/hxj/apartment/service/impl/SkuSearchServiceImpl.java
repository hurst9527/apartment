...
import hxj.apartment.dao.SkuSearchMapper;
...
//            nativeSearchQueryBuilder.withPageable(PageRequest.of(currPageNo - 1, pageSize));
        ...
        if (searchMap != null && searchMap.size() > 0) {
            ...
            if (!StringUtils.isEmpty(spuID)) {
                boolQueryBuilder.must(QueryBuilders.termQuery("spuId", spuID));
            }
    ...
    public void importData2Es() {
        ...
        skuSearchMapper.saveAll(skuInfoList);
    }
    ...
    public void insert(skuInfo skuInfo) {
        String spec = skuInfo.getSpec();
        skuInfo.setSpecMap(JSON.parseObject(spec, Map.class));
        skuSearchMapper.save(skuInfo);
    }

    @Override
    public void delete(Long skuInfoID) {
        skuSearchMapper.deleteById(skuInfoID);
    }
